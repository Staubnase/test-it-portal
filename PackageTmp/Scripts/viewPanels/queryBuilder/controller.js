    ...
    var definition = {
        ...
        build: function (vm, node, callback) {
            ...
            for (var i = 0; i < schemaDef.sections.length; i++) {
                ...
                //restrict search to incidents only on free version
                if (!session.consoleSetting.AnalystPortalLicense.IsValid) {
                    if (sectionDef.displayKey.toLowerCase() === "incidents") {
            ...
            var getSectionSchemaFields = function (sectionTypeName) {
                ...
                return !_.isUndefined(section) ? section.fields : [];
            }
            ...
            var viewModel = kendo.observable({
                ...
                getRelativeDate: function(relativeType, offsetValue) {
                    ...
                    switch (relativeType) {
                        case "tomorrow":
                            relativeDate.setDate(currentDate.getDate() + 1);
                            break;
                        case "daysfromnow":
        ...
        "gridDefaultConfig": {
            ...
            grid: {
                ...
                columns: [
                    ...
                    { "DataType": "string", "field": "Title", "width": 350, "attributes": { "class": "grid-highlight-column grid-highlight-column-title" }, template: "<a href='#var url = app.gridUtils.getLinkUrl(data, 'WorkItem');##=url#' onclick='return false;'>#: Title # </a>" },
