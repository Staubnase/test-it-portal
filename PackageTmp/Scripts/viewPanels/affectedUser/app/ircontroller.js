// CONTROLLERS
affectedUserApp.controller('incidentController', ['$scope', '$location', 'valueService', 'listService', 'flatListService', 'createWIService', 'templateListService', 'userListService', 'userService', 'enumMatchService', function($scope, $location, valueService, listService, flatListService, createWIService,  templateListService, userListService, userService, enumMatchService){
    $('#createIRTaskbar').show();
    $('#createSRTaskbar').hide();
    ...
    $scope.irtemplateid = valueService.irtemplateid;  
    $scope.title = valueService.title;  
    $scope.description = valueService.description;
    $scope.altcontact = valueService.altcontact;
    $scope.tierqueue = valueService.tierqueue;   
    $scope.classification = valueService.classification;  
    $scope.irsource = valueService.irsource; 
    $scope.impact = valueService.impact; 
    $scope.irurgency = valueService.irurgency; 
    
    $scope.$watch('[irtemplateid, title, description, altcontact]', function(){   
        valueService.irtemplateid = $scope.irtemplateid;
        valueService.title = $scope.title;
        valueService.description = $scope.description;
        valueService.altcontact = $scope.altcontact;
    });
    
    $scope.$watch('[tierqueue]', function(){   
        valueService.tierqueue = $scope.tierqueue;

        if(valueService.supportgroup != null){
            var jsonObject = enumMatchService.checkForMatch(valueService.supportgroup, valueService.tierQueueList); // Find index of matching enum value in other WI's corresponding list
        
            if(jsonObject != null){
                valueService.tierqueue = jsonObject; // Set the valueService value (persistent)
                $scope.tierqueue = jsonObject; // Set the scope value (needed to render current form)
            }
            ...
            valueService.supportgroup = null; // When we are done with the comparison, we will clear the other page's value
        }
    });
    
    $scope.$watch('[irsource]', function(){     
        valueService.irsource = $scope.irsource;
        
        if(valueService.srsource != null){
            var jsonObject = enumMatchService.checkForMatch(valueService.srsource, valueService.irSourceList); // Find index of matching enum value in other WI's corresponding list

            if(jsonObject != null){
                valueService.irsource = jsonObject; // Set the valueService value (persistent)
                $scope.irsource = jsonObject; // Set the scope value (needed to render current form)
            }
            ...
            valueService.srsource = null; // When we are done with the comparison, we will clear the other page's value
        }
    });
    
    $scope.$watch('[classification]', function(){     
        valueService.classification = $scope.classification;
        
        if(valueService.area != null){
            var jsonObject = enumMatchService.checkForMatch(valueService.area, valueService.classificationList); // Find index of matching enum value in other WI's corresponding list

            if(jsonObject != null){
                valueService.classification = jsonObject; // Set the valueService value (persistent)
                $scope.classification = jsonObject; // Set the scope value (needed to render current form)
            }
            ...
            valueService.area = null; // When we are done with the comparison, we will clear the other page's value
        }
    });
    
    $scope.$watch('[impact]', function(){     
        valueService.impact = $scope.impact;
        
        if(valueService.priority != null){
            var jsonObject = enumMatchService.checkForMatch(valueService.priority, valueService.impactList); // Find index of matching enum value in other WI's corresponding list

            if(jsonObject != null){
                valueService.impact = jsonObject; // Set the valueService value (persistent)
                $scope.impact = jsonObject; // Set the scope value (needed to render current form)
            }
            ...
            valueService.priority = null; // When we are done with the comparison, we will clear the other page's value
        }
    });
    
    $scope.$watch('[irurgency]', function(){     
        valueService.irurgency = $scope.irurgency;
        
        if(valueService.srurgency != null){
            var jsonObject = enumMatchService.checkForMatch(valueService.srurgency, valueService.irUrgencyList); // Find index of matching enum value in other WI's corresponding list

            if(jsonObject != null){
                valueService.irurgency = jsonObject; // Set the valueService value (persistent)
                $scope.irurgency = jsonObject; // Set the scope value (needed to render current form)
            }
            ...
            valueService.srurgency = null; // When we are done with the comparison, we will clear the other page's value
        }
    });
    ...
    $scope.submit = function(){
        ...
        var creationResult = createWIService.createProjectionByCriteria("IR", $scope.title, $scope.description, $scope.altcontact, $scope.classification, $scope.tierqueue, $scope.irtemplateid, $scope.irsource, $scope.enduserobj, $scope.assigneduserobj, $scope.irurgency, $scope.impact);

        creationResult.then(function (result) {
            $scope.successType = "Incident"
            $scope.successLink = '/Incident/Edit/' + result
            $scope.successId = result;

            app.lib.message.add(localization.IncidentSavedMessage + "&nbsp;&nbsp;<a href='" + $scope.successLink + "'><strong>" + $scope.successId + "</strong></a> ", "success");
