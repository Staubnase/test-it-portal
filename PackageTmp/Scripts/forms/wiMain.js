...
require(["forms/taskBuilder", "forms/headerBuilder", "forms/formBuilder", "forms/formHelper"], function (taskBuilder, headerBuilder, formBuilder, formHelper, messagesStructure) {
    ...
    var init = function () {
        ...
        if (!pageForm.newWI) {
            ...
            notArchived = typeof (pageForm.viewModel.CiresonAnalyticsArchive) == 'undefined' || !pageForm.viewModel.CiresonAnalyticsArchive
            if (!notArchived) {
                pageForm.viewModel.isDisabled = true;
            }
    ...
    var sendTeamsNotification = function (fromJson)
    {
        if (session.features?.TeamsNotification) {
            $.ajax({
                url: "/TeamsNotification/NotifyUser",
                type: "POST",
                data: "formJson=" + fromJson,
                error: function (jqXHR, textStatus, errorThrown) {
                    console.log(jqXHR, textStatus, errorThrown);
                }
            });
    ...
    var save = function (success, failure) {
        ...
        if (pageForm.type === 'ChangeRequest') {
            ...
            if (pageForm.jsonRaw.Status.Id != ChangeStatus_Closed && dataVM.Status.Id == ChangeStatus_Closed) {
                dataVM.ActualEndDate = new Date().toISOString();
            }
        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {
                ...
                sendTeamsNotification(postData);
    ...
    var saveActionCommentLog = function (success, failure) {
        var current = pageForm.jsonRaw;
        current.AppliesToTroubleTicket = pageForm.viewModel.AppliesToTroubleTicket;
        current.AppliesToWorkItem = pageForm.viewModel.AppliesToWorkItem;
    ...
    pageForm.saveActionCommentLog = saveActionCommentLog;
